...
export default async function PublicProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  try {
    ...
    const user = await User.findOne({ username: sanitizedUsername }).select('username clerkId imageUrl bio social roles points').lean() as any;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.keys(defaultSocial).map((platform) => {
                    ...
                    return (
                      <div
                        key={platform}
                        className={`flex items-center p-3 rounded-lg ${link ? 'bg-purple-900/20' : 'bg-neutral-900/50'
                          }`}
                      >
